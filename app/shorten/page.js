    ...
    return (
        <div className='mx-auto max-w-lg bg-blue-100 my-16 p-8 rounded-lg flex flex-col gap-4'>
            ...
            <div className='flex flex-col gap-2'>
                <input type="text"
                    value={url}
                    className='px-4 py-2 focus:outline-blue-400 bg-blue-50 rounded-md'
                    placeholder='Enter your URL' onChange={e => { seturl(e.target.value) }} />

                <input type="text"
                    value={shorturl}
                    className='px-4 py-2 focus:outline-blue-400 bg-blue-50 rounded-md'
                    placeholder='Enter your preferred short URL' onChange={e => { setshorturl(e.target.value) }} />
                <button onClick={generate} className='bg-blue-500 rounded-lg shadow-lg p-3 py-1 font-bold cursor-pointer hover:bg-blue-300 text-white my-3'>Generate</button>
            </div>
